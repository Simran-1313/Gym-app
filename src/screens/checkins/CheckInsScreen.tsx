...
const getMethodColor = (colors: { accent: string; primary: string; success: string }) => ({
...
const CheckInCard: React.FC<{ item: CheckIn; index: number }> = ({ item, index }) => {
  ...
  const color = mColors[item.method as keyof typeof mColors] ?? colors.primary;
      ...
      <GlassCard glowColor={color} style={styles.card}>
        <View style={styles.cardLeft}>
          ...
          <View style={styles.details}>
            <Text style={[styles.inTime, { color: colors.text }]}>{formatDateTime(item.checkedInAt)}</Text>
            {item.checkedOutAt ? (
              <Text style={[styles.outTime, { color: colors.textSecondary }]}>Out: {formatDateTime(item.checkedOutAt)}</Text>
            ) : (
              <View style={styles.activeBadge}>
                <View style={[styles.activeDot, { backgroundColor: colors.success }]} />
                <Text style={[styles.activeText, { color: colors.success }]}>Currently inside</Text>
              </View>
            )}
        ...
        <View style={styles.durationBox}>
          ...
          <Text style={[styles.durationText, { color: colors.textSecondary }]}>{duration(item.checkedInAt, item.checkedOutAt)}</Text>
        </View>
      </GlassCard>
    </StaggerItem>
  );
...
const styles = StyleSheet.create({
  ...
  inTime: { fontSize: FONT_SIZE.sm, fontWeight: '600' },
  outTime: { fontSize: FONT_SIZE.xs },
  activeBadge: { flexDirection: 'row', alignItems: 'center', gap: 6 },
  activeDot: { width: 8, height: 8, borderRadius: 4 },
  activeText: { fontSize: FONT_SIZE.xs, fontWeight: '600' },

  durationBox: { alignItems: 'flex-end', gap: 2 },
  durationText: { fontSize: FONT_SIZE.xs },
